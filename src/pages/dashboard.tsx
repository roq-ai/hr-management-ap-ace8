...
import { useRoqPlatformUserProfiles, usePerformanceReviewCount, useRecruitmentCount, useTrainingCount } from 'lib/roq';

import { TrainingListPage as ShowCaseList } from 'pages/trainings';
...
function DashboardPage() {
  ...
  const recruitmentStat = useRecruitmentCount();

  const trainingStat = useTrainingCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Recruitments',
      result: recruitmentStat?.data,
      isLoading: recruitmentStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Trainings',
      result: trainingStat?.data,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`trainings`} passHref legacyBehavior>
                  <Button
